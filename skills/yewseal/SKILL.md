...
- **SOPS config synchronization** — `init` and `encrypt` default to generating `.sops.yaml`; `encrypt` always uses the complete resolved project policy even when its targets select one mapping. Use `--sync-sops-config=false` to leave it untouched.
