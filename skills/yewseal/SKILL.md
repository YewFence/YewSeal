...
- **SOPS config synchronization** — `init` and `encrypt` default to generating `.sops.yaml`; `encrypt` always uses the complete resolved project policy even when its targets select one mapping. It finishes ciphertext work before synchronization, but a synchronization failure still makes the command fail. Use `--sync-sops-config=false` to leave it untouched.
