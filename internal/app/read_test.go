	...
	for _, tc := range []struct {
		...
		for _, strict := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/strict=%v", tc.name, strict), func(t *testing.T) {
				...
				result, err := DiffPlaintextAgainstEncryptedTargets(&out, &diagnostics, cfg, "", env.keyFile, true, "never", strict)
...
func TestReadCommandsPreserveHistoryAndOutputChannels(t *testing.T) {
	...
	var out, diagnostics bytes.Buffer
	result, err := DiffPlaintextAgainstEncryptedTargets(&out, &diagnostics, cfg, "secrets", env.keyFile, true, "never", true)
	require.NoError(t, err)
	require.True(t, result.Different)
	require.Contains(t, diagnostics.String(), "warning:")
...
func TestDiffAlwaysLoadsIdentityBeforeMissingInputSkips(t *testing.T) {
	...
	var out, diagnostics bytes.Buffer
	_, err := DiffPlaintextAgainstEncryptedTargets(&out, &diagnostics, cfg, "", filepath.Join(t.TempDir(), "missing-key"), false, "never", false)
	require.Error(t, err)
	require.Empty(t, out.String())
...
func TestReadCommandsPropagateOutputErrors(t *testing.T) {
		...
		for _, match := range []string{"warning:", "Selected", "Reading", "Decrypting", ""} {
			t.Run(command+"/"+match, func(t *testing.T) {
				...
				if command == "view" {
					...
					_, err = DiffPlaintextAgainstEncryptedTargets(&out, rejectedOutput{match: match}, cfg, "", env.keyFile, true, "never", false)
				}
	...
	_, err = DiffPlaintextAgainstEncryptedTargets(rejectedOutput{}, io.Discard, cfg, "", env.keyFile, false, "never", false)
	require.ErrorIs(t, err, errReadOutput)
	_, err = DiffPlaintextAgainstEncryptedTargets(io.Discard, rejectedOutput{match: "Summary"}, cfg, "", env.keyFile, false, "never", false)
	require.ErrorIs(t, err, errReadOutput)
}
