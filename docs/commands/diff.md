...
```bash
yews diff [command options] [path-or-pattern]...
```

## 目标选择

不传位置参数时，`diff` 按明文路径选择当前目录范围内的映射。位置参数可以是已发现映射的明文或密文路径、已存在的目录（只筛选该目录明文侧的已登记映射，不重新扫描），或含 `*`、`?` 等元字符的模式（明文或密文路径命中均可）；多个参数取并集，任一参数没有命中都会报错。
